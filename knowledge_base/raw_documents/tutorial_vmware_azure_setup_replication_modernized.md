# Set up VMware VM disaster recovery to Azure with Azure Site Recovery - Modernized - Azure Site Recovery | Microsoft Learn
...
For information on how to set up disaster recovery in Azure Site Recovery Classic releases, see [the tutorial](vmware-azure-tutorial).

This is the second tutorial in a series that shows you how to set up disaster recovery to Azure for on-premises VMware VMs. In the previous tutorial, we [prepared the on-premises Azure Site Recovery replication appliance](deploy-vmware-azure-replication-appliance-modernized) for disaster recovery to Azure.
...
The classic experience to protect VMware machines by using ASR has retired on March 30, 2026. [Learn more](/en-us/azure/site-recovery/vmware-physical-azure-classic-deprecation). Switch to the [modernized experience](/en-us/azure/site-recovery/move-from-classic-to-modernized-vmware-disaster-recovery) to avoid service interruption.
...
- [Create a recovery Services vault](quickstart-create-vault-template?tabs=CLI)
- Prepare infrastructure - [deploy an Azure Site Recovery replication appliance](deploy-vmware-azure-replication-appliance-modernized)
- Enable replication
...
- Resource Manager based storage accounts (Standard Type):
    ...
    - [Storage Blob Data Contributor](../role-based-access-control/built-in-roles#storage-blob-data-contributor)
- Resource Manager based storage accounts (Premium Type):
    - [Contributor](../role-based-access-control/built-in-roles#contributor)
    - [Storage Blob Data Owner](../role-based-access-control/built-in-roles#storage-blob-data-owner)
...
You need to [set up an Azure Site Recovery replication appliance on the on-premises environment](deploy-vmware-azure-replication-appliance-modernized) to channel mobility agent communications.
...
Ensure the [pre-requisites](vmware-physical-azure-support-matrix) across storage and networking are met.
...
5. After you select the list of VMs, select **Next** to proceed to source settings. Here, select the replication appliance and VM credentials. These credentials will be used to push mobility agent on the machine by Azure Site Recovery replication appliance to complete enabling Azure Site Recovery. Ensure accurate credentials are chosen.
    ...
    In addition to automatic installation, Mobility Services agent can be installed manually or through an automated deployment method, such as Configuration Manager. [Learn more](vmware-physical-mobility-service-overview).
