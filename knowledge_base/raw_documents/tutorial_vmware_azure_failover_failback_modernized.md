# Run VMware VMs failover to Azure - Azure Site Recovery | Microsoft Learn

This article describes how to fail over an on-premises VMware virtual machine (VM) to Azure with [Azure Site Recovery](site-recovery-overview) - Modernized.

For information about failover in Classic releases, see [this article](vmware-azure-tutorial-failover-failback).
...
Tutorials show you the simplest deployment path for a scenario. They use default options where possible and don't show all possible settings and paths. If you want to learn about failover in detail, see [Fail over VMs and physical servers](site-recovery-failover).
...
1. Make sure you've [set up Azure](tutorial-prepare-azure) for on-premises disaster recovery of VMware VMs.
2. Prepare your on-premises [VMware](vmware-azure-tutorial-prepare-on-premises) environment for disaster recovery.
3. Set up disaster recovery for [VMware VMs](vmware-azure-set-up-replication-tutorial-modernized).
4. Run a [disaster recovery drill](tutorial-dr-drill-azure) to make sure that everything's working as expected.

## Verify VM properties

Before you run a failover, check the VM properties to make sure that the VMs meet [Azure requirements](vmware-physical-azure-support-matrix#replicated-machines).
...
3. The URLs mentioned [here](vmware-azure-architecture-modernized#set-up-outbound-network-connectivity) are accessible from the VM.
