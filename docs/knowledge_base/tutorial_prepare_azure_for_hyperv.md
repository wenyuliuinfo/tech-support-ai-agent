# Prepare Azure resources for Hyper-V disaster recovery by using Azure Site Recovery - Azure Site Recovery | Microsoft Learn

[Azure Site Recovery](site-recovery-overview) helps organizations with business continuity and disaster recovery by keeping business apps running during planned and unplanned outages. Site Recovery manages and orchestrates disaster recovery of on-premises machines and Azure virtual machines (VMs), including replication, failover, and recovery.
...
2. On the **Categories** menu, select **Storage**, and then select **Storage account**.
3. On **Create a storage account**, on the **Basics** tab, complete these steps:

    1. For **Subscription**, select the Azure subscription in which to create the storage account.
    2. For **Resource group**, select **Create new**, and then enter a new resource group name. For example, enter **ContosoRG**.

        An Azure resource group is a logical container in which Azure resources are deployed and managed.
    3. For **Storage account name**, enter a name for the account. For example, enter **contosovmsacct1910171607**.
    ...
    4. For **Region**, select the geographic location for your storage account. For example, select **West Europe**.
    5. For **Performance**, select **Standard**.
    6. For **Redundancy**, retain the default **Geo-redundant storage (GRS)** for storage redundancy.

    ![Screenshot that shows an example of Create a storage account option.](media/tutorial-prepare-azure/create-storage-account.png)
4. Select **Review** and review your settings.
5. Select **Create**.
...
![Screenshot that shows the Create a legacy storage account link.](media/tutorial-prepare-azure/create-legacy-storage-account.png)
...
3. In the search results, select **Backup and Site Recovery**. Next, select **Create**.
4. On **Create Recovery Services vault**, on the **Basics** tab, complete these steps:

    1. For **Subscription**, select the subscription in which to create the new Recovery Services vault.
    2. For **Resource group**, select an existing resource group or create a new one. For example, select **contosoRG**.
    3. For **Vault name**, enter a name you can use to identify the vault. For example, enter **ContosoVMVault**.
    4. For **Region**, select the region where the vault should be located. For example, select **West Europe**.

    ![Screenshot that shows the Create Recovery Services vault pane.](media/tutorial-prepare-azure/new-vault-settings.png)
5. Select **Review + create**, and then select **Create** to create the recovery vault.
...
1. In the [Azure portal](https://portal.azure.com), select **Create a resource**.
2. On the **Categories** menu, select **Networking**, and then select **Virtual Network**.
3. On **Create virtual network**, on the **Basics** tab, complete these steps:

    1. For **Subscription**, select the subscription in which to create the network.
    2. For **Resource group**, select the resource group in which to create the network. For this tutorial, select the existing resource group **contosoRG**.
    3. For **Name**, enter a name for the network. The name must be unique within the Azure resource group. For example, enter **ContosoASRnet**.
    4. For **Region**, select **West Europe**. The network must be in the same region as your Recovery Services vault.

    ![Screenshot that shows the Create virtual network options.](media/tutorial-prepare-azure/create-network.png)
4. On **Create virtual network**, on the **IP addresses** tab, complete these steps:
    ...
    1. Because there's no subnet for this network, first delete the existing address range. To delete the range, select the ellipsis (**...**), under the available IP address range, and then select **Delete address space**.
        ...
        ![Screenshot that shows deleting the address space.](media/tutorial-prepare-azure/delete-ip-address.png)
    2. After you delete the existing address range, select **Add an IP address space**.

        ![Screenshot that shows adding an IP address space.](media/tutorial-prepare-azure/add-ip-address-space.png)
    3. For **Starting address**, enter **10.0.0**.
    4. For **Address space size**, select **/24 (256 addresses)**.

        ![Screenshot that shows the Add virtual network options.](media/tutorial-prepare-azure/homepage-ip-address.png)
5. Select **Add**.
6. Select **Review + create**, and then select **Create** to create a new virtual network.
